                ...
                <div id={style.imageArea}>
            {
                getImages.map((item, idx) => (
                    <div className={style.image} key={idx} >
                        ...
                        <button style={{backgroundColor: "#FF746C"}} className={`basicButton`} onClick={() => handleDelete(item)}>Delete</button>
                    </div>
                ))
            }
           
            </div>
            <button onClick={handleSubmit} className="basicButton">Confirm Changes</button>
